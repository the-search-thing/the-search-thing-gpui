//! Multi-line text field + IME bridge (`EntityInputHandler`) and editing keybindings.
//! Pattern matches `gpui-ce` `examples/input.rs`.
...
pub struct TextInput {
    ...
    /// Placeholder text color for empty field. If `None`, uses [`Window::text_style`] color at reduced opacity (matches light/dark theme).
...
impl TextInput {
    pub fn new(cx: &mut Context<'_, Self>, placeholder: impl Into<SharedString>) -> Self {
        Self {
    ...
    /// Override placeholder color (see [`Self::placeholder_color`]).
...
impl gpui::Element for TextElement {
    ...
    fn prepaint(
        ...
        let (display_text, text_color) = if content.is_empty() {
            let placeholder_clr = input
                .placeholder_color
                .unwrap_or_else(|| style.color.opacity(0.35));
            (input.placeholder.clone(), placeholder_clr)
        } else {
            (content, gpui::white())
        };
...
/// Register keystrokes that dispatch to [`TextInput`]'s `.on_action` handlers.
/// Call once from [`gpui::Application::run`] before opening the window.
pub fn register_text_input_keybindings(cx: &mut gpui::App) {
    cx.bind_keys([
